...
async function loadProgramming() {
    ...
    try {
        ...
        list.innerHTML = collection.map((item) => {
            ...
            const dateTimeStr = formatLocalizedDateTime(item.dateTime, lang);
...
function formatLocalizedDateTime(value, lang) {
    ...
    const monthsMap = {
        ...
        agost: { es: "agosto", va: "agost", en: "August", fr: "août" },
        apr: { es: "abr", va: "abr", en: "Apr", fr: "avr" },
        aug: { es: "ago", va: "ago", en: "Aug", fr: "août" }
    };

    const targetLang = ["es", "va", "en", "fr"].includes(lang) ? lang : "es";

    Object.keys(daysMap).forEach((dayKey) => {
        const regex = new RegExp(`\\b${dayKey}\\b`, "gi");
        str = str.replace(regex, daysMap[dayKey][targetLang]);
    });

    Object.keys(monthsMap).forEach((monthKey) => {
        const regex = new RegExp(`\\b${monthKey}\\b`, "gi");
        str = str.replace(regex, monthsMap[monthKey][targetLang]);
    });
