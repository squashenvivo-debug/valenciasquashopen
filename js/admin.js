...
async function insertUploadedImageIntoArticle(targetInputId, file, triggerInputEl) {
    ...
    try {
        ...
        const tag = `<img src="${imageSrc}" alt="Imagen" style="max-width:100%; max-height:480px; width:auto; height:auto; display:block; margin:15px auto; border-radius:8px;">`;
    ...
    host.innerHTML = newsItems.map((item) => {
        ...
        <article class="gallery-admin-card" data-news-id="${item.id}">
            ...
            <div class="wysiwyg-toolbar" style="display:flex; flex-wrap:wrap; gap:6px; margin-bottom:8px; padding:6px; background:#081a2a; border:1px solid #1a2a3a; border-radius:6px;">
                ...
                <button type="button" class="wysiwyg-btn" onclick="document.getElementById('newsArticleImageUpload_${item.id}').click()" style="padding:4px 10px; background:#2e7d32; color:#fff; border:1px solid #4caf50; border-radius:4px; cursor:pointer;" title="Subir una foto desde tu ordenador o móvil e insertarla aquí">📷 Subir imagen</button>
                <input id="newsArticleImageUpload_${item.id}" type="file" accept="image/*" style="display:none" onchange="insertUploadedImageIntoArticle('newsArticle_${item.id}_es', this.files[0], this)">
                <button type="button" class="wysiwyg-btn" onclick="insertFormatTag('newsArticle_${item.id}_es', '<img src=&quot;', '&quot; alt=&quot;Imagen&quot; style=&quot;max-width:100%; max-height:480px; width:auto; height:auto; display:block; margin:15px auto; border-radius:8px;&quot;>')" style="padding:4px 10px; background:#122b42; color:#81C784; border:1px solid #234567; border-radius:4px; cursor:pointer;" title="Ya tengo la URL de la imagen">🖼️ Imagen (URL)</button>
