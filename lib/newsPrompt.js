...
/**
 * Prompt para el "cuadro de chat": el redactor pide un cambio concreto sobre un
 * artículo ya generado (p.ej. "hazlo más corto", "quita la última frase", "dale
 * más protagonismo a Iván Pérez"), y el modelo devuelve el artículo completo
 * actualizado, manteniendo la voz de esa versión y todo lo que no se pidió cambiar.
 */
function buildRefinePrompt({ versionId, article, instruction, language }) {
    const style = VERSION_STYLES.find((entry) => entry.id === versionId) || VERSION_STYLES[0];
    const languageName = LANGUAGE_NAMES[language === "en" ? "en" : "es"];

    const system = `${style.systemPrompt}
${SHARED_SYSTEM_SUFFIX}

Ahora actúas como editor sobre un artículo que ya escribiste. El redactor te pide un cambio
concreto. Aplica exactamente ese cambio y deja todo lo demás igual salvo que el cambio
pedido lo requiera (por ejemplo, si piden "hazlo más corto", sí puedes recortar varias
partes). No añadas datos, cifras ni citas que no estuvieran ya en el artículo original.`;

    const user = `Artículo actual (JSON):
${JSON.stringify(article, null, 2)}

Cambio pedido por el redactor: "${instruction}"

Devuelve el artículo completo actualizado en ${languageName}, en el mismo formato JSON.`;
