...
/** Junta los jugadores de todas las divisiones en un único mapa id -> info (forma plana). */
    ...
    divisions.forEach((division) => {
        ...
        players.forEach((player) => {
            ...
            map.set(String(player.id), {
                ...
                ranking: player.current_world_ranking ?? null,
...
function isWildcardOrQualifier(info) {
    if (info?.is_wildcard) return true;
...
/**
 * Cuando todavía no hay partidos completados (antes de que arranque el torneo,
 * o justo después de la última noticia), construye una "previa" a partir del
 * cuadro: cabezas de serie, país, ranking. Sirve tanto para el primer artículo
 * del torneo como para probar el generador sin esperar a que haya resultados.
 */
function buildTournamentPreview(divisions) {
    const playerInfoMap = buildPlayerInfoMap(divisions);
    const seededPlayers = Array.from(playerInfoMap.values())
        .filter((player) => Number(player.seed_number) > 0)
        .sort((a, b) => Number(a.seed_number) - Number(b.seed_number))
        .slice(0, 6);

    const drawSize = (Array.isArray(divisions) ? divisions : [])
        .reduce((max, division) => Math.max(max, Number(division?.draw_size) || 0), 0);

    const topSeed = seededPlayers[0] || null;
    const brief = topSeed
        ? `El cuadro está encabezado por ${topSeed.name}${topSeed.country ? ` (${topSeed.country})` : ""} como cabeza de serie nº${topSeed.seed_number}, junto a otros ${Math.max(seededPlayers.length - 1, 0)} sembrados de nivel.`
        : "Aún no hay partidos disputados: esta es una previa general del torneo a partir del cuadro.";

    return {
        angleKey: "tournament_preview",
        mainStoryLabel: "previa del torneo",
        mainStoryBrief: brief,
        mainMatch: null,
        playerOfDay: topSeed?.name || null,
        totalMatchesAnalyzed: 0,
        allFavoritesWon: false,
        topMatches: [],
        topSeeds: seededPlayers.map((player) => ({
            name: player.name,
...
module.exports = { analyzeStory, buildTournamentPreview, buildPlayerInfoMap, MARATHON_MINUTES };
